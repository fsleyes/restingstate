...
data <- rawData %>% 
  ...
  mutate(diff_pair1 = post_pair1 - pre_pair1,
         ...
         diff_pair1_flip = pre_pair1 - post_pair1)
...
```{r}
reg_target_pair1 <- lm(diff_target ~ diff_pair1_flip, data = data)
...
```{r}
ind_learn_scatter <- ggplot(data, aes(x = diff_pair1_flip, y = induction_learning)) +
  geom_point() +
